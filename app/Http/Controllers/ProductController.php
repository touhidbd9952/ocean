...
class ProductController extends Controller
{
    ...
    { 
        ...
        $product_woners = Product_woner::where('status','active')->get(); 
        return view('backend.admin.product.details_view_product',compact('product','categories','brands','product_woners'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            //"long_description" => 'required',
            //"long_description_jp" => 'required',
            //"delivery_place" => 'max:255',
            "releasing_charge" => 'max:20',
            'thumbnail_image' => 'mimes:jpg,jpeg,png,gif,JPG,JPEG,PNG,GIF',
            'conditional_report' => 'mimes:pdf,txt,rtf,jpg,jpeg,png,gif,JPG,JPEG,PNG,GIF',
        ]);
    ...
    public function get_maximum_bidder_list(Request $request)
    {
       ...
       if(count($products)>0)   
       {
         ...
         if($auction_max_bidder_ids !="")
         {
            ...
            return view('backend.admin.product.auction_bidderlist_for_invoice',compact('auctionmaxbidderlist','enddate'));
         }
         else
         {
    ...
    public function bidder_winner_invoice($id,$enddate)
    {
        $this->checkvaliduser();
        ...
        $invoiceno = 'WAEB'.$auctionYear.$auctionMonth.$auctionDay.rand(1000000,9999999); 
        return view('backend.admin.product.customer_fax',compact('products','auctionmaxbidderinfo','invoiceno','enddate','auctionYear','auctionMonth','auctionDay','deliveryplaces'));
    }
    ...
    public function get_sold_product_owner_list(Request $request)
    {
       ...
       if(count($products)>0)   
       {
         ...
         if($auction_product_owner_ids !="")
         {
            ...
            return view('backend.admin.product.auction_product_owner_list_for_invoice',compact('auctionproductownerlist','enddate'));
         }
         else
         {
            return Redirect()->back()->with('esuccess',"No data found");
    ...
    public function product_owner_invoice($id,$enddate)
    {
        $this->checkvaliduser();
        ...
        $auctionMonth = date('m', $time_input);
        $auctionDay = date('d', $time_input);
        $invoiceno = 'WAEO'.$auctionYear.$auctionMonth.$auctionDay.rand(1000000,9999999); 
        
        $commissionpercent = 5; 
        return view('backend.admin.product.auction_product_owner_fax',compact('products','auctionproductownerinfo','invoiceno','commissionpercent','enddate','auctionYear','auctionMonth','auctionDay'));
    }
