...
input.myclass{font-family: 'password';}
    ...
    <div id="login"> 
        ...
        <dd>
            <input type="search" id="username" autocomplete="off" style="border: #fbe8d0;padding-left: 5px;color: #000;min-width: 167px;min-height:25px;" required>
        </dd>
        <dt style="color: #F5A64E;">{{$pagelanguage['LG_Password']}}</dt>
        <dd>
            <input type="text"  id="userpass" class="myclass"  autocomplete="off"   style="border: #fbe8d0;padding-left: 5px;color: #000;min-width: 167px;min-height:25px;" required>
        </dd>
        </dl>
    ...
    <div id="bidder_section" class="bidderselection">  
        <form id="favorite" method="post" action="">
            <a href="{{route('auction.selected_product')}}" class="button"><?php if(session()->get('language')=='en'){?>Selections<?php }else{?>セレクション<?php }?></a>
        ...
        <a href="{{url('auction/bidder_logout')}}" style="padding-left: 10px;"><i class="fa fa-sign-out"></i>Logout</a>
    </div>
    ...
    <div class="rd-navbar-nav-wrap__inner">
      ...
      <div class="rd-navbar-nav-wrap__inner" style="margin-top: 15px;">
        ...
        <!-- Navbar Nav-->
        <ul>
            <a href="javascript:" class="menutitle">Consignor</a>
            <li>
                <?php 
                $auctiondata = App\Models\Auction::where('result_show','=',1)->get(); 
            if(count($auctiondata)>0)
            {
                $auction_end = $auctiondata[0]->auction_end;
                //$auction_date =  date('Y-m-d',strtotime(App\Models\Auction::max('auction_end')->where('status','!=',1)->get())); 
                $last_auction_date = App\Models\Product::where(function ($query) use ($auction_end) {
                    // subqueries goes here
                    $query->where('auction_end','like', '%'.$auction_end.'%');
                })
                ->max('auction_end');
                if(date('Y-m-d',strtotime($last_auction_date)))
                {
                ?>
                <a href="{{route('woody.bid_result')}}">Results of {{date('Y-m-d',strtotime($last_auction_date))}}</a>
                <?php 
                }
            }
            ?>
